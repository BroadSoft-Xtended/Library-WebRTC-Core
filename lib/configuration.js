...
function Configuration(options, eventbus, debug) {
  ...
  var getVideoConstraints = function() {
    if (isAudioOnlyView() || self.audioOnly) {
      return false;
    } else {
  ...
  self.init = function(options) { 
    ...
    if (self.features) {
      self.setFeatures(parseInt(self.features, 10));
    }
  };
    ...
    var options = {
      ...
      createOfferConstraints: {
        mandatory: {
          ...
          OfferToReceiveVideo: !isAudioOnlyView() && offerToReceiveVideo
        }
      }
    };
    return options;
  ...
  self.getResolutionDisplay = function() {
    return isHD() ? WebRTC_C.R_1280x720 : self.displayResolution;
  }  
