...
function SIPStack(eventbus, debug, urlconfig, cookieconfig, screenshare) {
  var self = {};
  ...
  self.bindings = {
    uaConfig: {
      sipstack: ['networkUserId', 'enableIms', 'domainFrom', 'debug', 'enableDatachannel', 'pAssertedIdentity'],
  ...
  self.listeners = function(databinder, screenshareDatabinder) {
    ...
    // TODO - no screenshareFailure event
    // eventbus.on("screenshareFailure", function(e) {
    //   self.reconnectUserMedia();
    // });
    screenshareDatabinder.onModelPropChange('visible', function(){
      if (screenshare.visible) {
    ...
    try {
      ...
      self.ua.on('newRTCSession', function(e) {
        ...
        if (e.data.session.direction === "incoming") {
          ...
          if (!self.activeSession) {
            debug.log('new active session : ' + session.id);
            self.activeSession = session;
          }
        }
      });
