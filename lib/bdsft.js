...
function View(constructor, options) {
	...
	self.create = function(constructorArgs, createOptions) {
		...
		if(!view) {
			console.error('no view template found : '+self.viewName, require('bdsft-webrtc-templates'));
			return;
		}
		...
		css.inject(self.viewName, createOptions);
		object.styleData = createOptions.styleData;

		var classes = object.view.attr('class');
		databinder(self.viewName).onModelPropChange('classes', function(value){
			object.view.attr('class', classes + ' ' + value.join(' '));
		});
		return object;
	};
